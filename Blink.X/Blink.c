...
#define _XTAL_FREQ 8000000 //Declaramos la frecuencia que ingresa al CPU

void main(void) {
    OSCCON = 0x7B; //Configuramos el oscilador interno a 8 MHz
    ADCON1 = 0x0F; //Configuramos todos los puertos como I/O Digitales
    TRISAbits.RA3 = 0; //Configuramos el pin RD7 como salida
    while(1){
        LATAbits.LATA3 = 1; //Configuramos el pin RD7 en alto
        __delay_ms(100); //Retardo de 200 ms
        LATAbits.LATA3 = 0;//Configuramos el pin RD7 en bajo
        __delay_ms(100);
    }
}
