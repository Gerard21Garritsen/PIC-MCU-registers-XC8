...
void display_Char(unsigned char caracter);
void display_String(uint8_t x,uint8_t y,unsigned char *cadena);
void display_Bitmap(void);
void display_Invert(uint8_t val);
...
void I2CInit(unsigned char modo,unsigned char speed){ //Funcion para habilitar el I2C
    //Configuramos los pinees del bus I2C
    TRISBbits.RB0 = 1;
    TRISBbits.RB1 = 1;
...
void display_Clear(void){ //Funcion para limpiar la pantalla
    for(uint8_t i = 0; i < 8; i++){ //Recorremos las 8 filas o paginas
        ...
        for(uint8_t j = 0; j < 132; j ++){ //Recorremos las columnas
            write_Cmd(0x40,0x00); //Ponemos en cero cada pixel
        }
    }
}
...
void display_Bitmap(void){ //Funcion para imprimir en pantalla un mapa de bits
    for(uint8_t i = 0; i < 8; i++){
        display_Set(0,i);
...
void display_Invert(unsigned char val){ //Funcion invertir los colores de la pantalla 0xA6 normal 0xA7 invertida
    write_Cmd(0x80,val);
}