...
char tf[8]; //Variable que almacena el periodo total del pulso en cadena
...
void main(void) {
    ...
    while(1){
        if(mostrar == 1){ //Mostrar la lectura del ancho del pulso en la OLED
            ...
            ltoa(tf,tt,10); //Conversion a una cadena de la lectura
            display_String(0,0,tf); //Mostramos en la OLED la lectura
...
void TIMERInit(void){
    ...
    PIR1bits.TMR1IF = 0;
    //Cargamos los TIMERS
    TMR0 = 252; //Contabilizar 25 ms de alto y bajo
    TMR1 = 0;
